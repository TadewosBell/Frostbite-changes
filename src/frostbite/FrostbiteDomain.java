...
public class FrostbiteDomain implements DomainGenerator {
	...
	public static final String SIZEATTENEMY = "size";
	...
	public static final String ENEMYCLASS = "enemy";
	/**
	...
	public static final String PFONENEMY = "pfOnEnemy";
	...
	private static final int enemySpeed = 4 * SCALE;
	private static int numberEnemyRow = 4;
	private static int numberPlatformRow = 4;
	private static int numberEnemyCol = 4;
	private static int Enemysize = 6 * SCALE;
	private static int spaceBetweenEnemy = 40 * SCALE;
	...
	protected static int buildingStepsToWin = 16;
	...
	public static void main(String[] args) {
		...
		RewardFunction rf = new FrostbiteRF(d);
		TerminalFunction tf = new FrostbiteTF(d);
		HashableStateFactory hashingFactory;
		Environment env = new SimulatedEnvironment(d, rf, tf, s);

		StateConditionTest goal = new TFGoalCondition(tf);
		...
		VisualActionObserver observer = new VisualActionObserver(d,vis);
		//VisualExplorer exp = new VisualExplorer(d, vis, s);
		//VisualExplorer exp = new VisualExplorer(d, env, vis);
		...
		//AStarDriver(outputPath, d, rf,goal, hashingFactory, s);

		//observer.initGUI();

		//env = new EnvironmentServer(env, observer);
		((SADomain)d).addActionObserverForAllAction(observer);
		AStarDriver(outputPath,tf, d, rf,goal, hashingFactory, s);

		Visualizer v = FrostbiteVisualizer.getVisualizer(fd);
		new EpisodeSequenceVisualizer(v, d, outputPath);

	}
	public static void AStarDriver(String outputPath,TerminalFunction tf, Domain d, RewardFunction rf, StateConditionTest goal, HashableStateFactory hashingFactory, State initialState) {

		Heuristic mdistHeuristic = new Heuristic() {
			@Override
			public  double h(State s) {
				ObjectInstance igloo = s.getObject("s");

				double cost = (Math.abs(buildingStepsToWin - igloo.getIntValForAttribute(BUILDINGATTNAME)));
		...
		DeterministicPlanner planner = new AStar(d, rf, goal, hashingFactory, mdistHeuristic);
	...
	public static void setEnemy(State s, int i, int x, int y, int ss) {
		ObjectInstance enemy = s.getObjectsOfClass(ENEMYCLASS).get(i);

		enemy.setValue(XATTNAME, x);
		enemy.setValue(YATTNAME, y);
		enemy.setValue(SIZEATTENEMY, ss);
	...
	/**
	 * initializes a full row of enemies
	 *
	 * @param d   domian
	 * @param s   the state to initialize the platforms in
	 * @param row the row id (starts at 0). Defines the direction platforms are going to move.
	...
	private static void setEnemyRow(Domain d, State s, int row) {
		for (int i = 0; i < numberEnemyCol; i++) {
			ObjectInstance enemy = new MutableObjectInstance(d.getObjectClass(ENEMYCLASS), ENEMYCLASS + (i + row * numberEnemyCol));
			s.addObject(enemy);

			enemy.setValue(XATTNAME, spaceBetweenEnemy * i + ((row % 4 == 0) ? 0 : gameWidth / 3));
			enemy.setValue(YATTNAME, gameIceHeight + jumpSize / 1 - Enemysize / 1 + agentSize / 1 + jumpSize * row);
			enemy.setValue(SIZEATTENEMY, Enemysize);
		}
	...
	public static State getCleanState(Domain domain) {
		...
		for (int i = 0; i < numberEnemyRow; i++)
			setEnemyRow(domain, s, i);
	...
	public Domain generateDomain() {
		...
		ObjectClass enemyClass = new ObjectClass(domain, ENEMYCLASS);
		enemyClass.addAttribute(xatt);
		enemyClass.addAttribute(yatt);
		enemyClass.addAttribute(satt);
		...
		new OnEnemyPF(PFONENEMY, domain);
	...
	private void update(State s, int leftToJump, boolean justLanded, int platformSpeedOnAgent) {
		...
		List<ObjectInstance> enemy = s.getObjectsOfClass(ENEMYCLASS);
		for (int i = 0; i < enemy.size(); i++) {
			int directionL = ((i / numberEnemyCol) % 2 == 0) ? 1 : -1;
			int x = enemy.get(i).getIntValForAttribute(XATTNAME) + directionL * enemySpeed;
			if (x < 0)
				x += gameWidth;
			enemy.get(i).setValue(XATTNAME, x % gameWidth);
		}
	...
	public class OnEnemyPF extends PropositionalFunction {


		public OnEnemyPF(String name, Domain domain) {
			super(name, domain, new String[]{AGENTCLASS, ENEMYCLASS});
		}

		@Override
		public boolean isTrue(State st, String... params) {
			ObjectInstance agent = st.getObject(params[0]);
			ObjectInstance enemy = st.getObject(params[1]);

			int x = enemy.getIntValForAttribute(XATTNAME);
			int y = enemy.getIntValForAttribute(YATTNAME);
			int s = enemy.getIntValForAttribute(SIZEATTENEMY);
...
//da			}
			...
			if (pointInPlatform(ax, ay, x, y, s)) {
				return true;
			}
