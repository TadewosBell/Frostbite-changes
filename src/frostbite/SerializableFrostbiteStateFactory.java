...
/**
 * A {@link SerializableStateFactory} for simple string representations of {@link FrostbiteDomain} states.
...
public class SerializableFrostbiteStateFactory implements SerializableStateFactory {
	...
	public static String stateToString(State s){
		...
		List<ObjectInstance> enemy = s.getObjectsOfClass(FrostbiteDomain.ENEMYCLASS);
		//write agent
		...
		for (ObjectInstance o : enemy) {
			buf.append("\n").append(o.getRealValForAttribute(FrostbiteDomain.XATTNAME)).append(" ");
			buf.append(o.getRealValForAttribute(FrostbiteDomain.YATTNAME)).append(" ");
			buf.append(o.getRealValForAttribute(FrostbiteDomain.SIZEATTENEMY)).append(" ");
	...
	public static State stringToState(Domain domain, String str){
		...
		for (int i = 2; i < lineComps.length; i++) {
			String[] oComps = lineComps[i].split(" ");
			FrostbiteDomain.setEnemy(s, i - 2, Integer.parseInt(oComps[0]), Integer.parseInt(oComps[1]),
					Integer.parseInt(oComps[2]));
		}
